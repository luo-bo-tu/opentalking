...
`datascale-ai/quicktalk` 已包含 QuickTalk、HuBERT 和 InsightFace `buffalo_l`，下载后应能看到 `checkpoints/auxiliary/models/buffalo_l/`。
...
如果依赖或算子不支持 MPS，请改用 `--backend mock` 验证产品流程，或把相同 `$DIGITAL_HUMAN_HOME/models/quicktalk/` 同步到 CUDA 机器运行。
